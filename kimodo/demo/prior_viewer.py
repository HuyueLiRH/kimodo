...
def load_prior_review_folder(run_folder: str | Path) -> PriorReviewFolder:
    ...
    for name, candidate_data in manifest.get("candidates", {}).items():
        ...
        variants = _prioritize_default_variant(variants, review_data.get("current_best_variant"))
...
def _prioritize_default_variant(
    variants: list[PriorMotionVariant],
    default_variant: str | None,
) -> list[PriorMotionVariant]:
    if not default_variant:
        return variants
    for index, variant in enumerate(variants):
        if variant.name == default_variant:
            return [variant, *variants[:index], *variants[index + 1 :]]
    return variants
